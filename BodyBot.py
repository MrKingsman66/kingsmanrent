...
async def confirm_order(callback: CallbackQuery):
    ...
    # Сохраняем заказ в базу данных SQLite
    ...
    # ОТПРАВЛЯЕМ ПАМЯТКУ ПОЛЬЗОВАТЕЛЮ
    await send_reminder_to_user(order_data["user_id"], order_data, order_id)
    ...
    await callback.message.edit_text(
        ...
        f"📋 <b>Вам отправлена памятка с правилами использования автомобиля</b>",
        parse_mode="HTML"
    )
...
async def send_reminder_to_user(user_id: int, order_data: dict, order_id: str):
    """Отправляет памятку пользователю после оформления заказа"""
    try:
        reminder_text = (
            "📋 <b>ПАМЯТКА ПО ИСПОЛЬЗОВАНИЮ АВТОМОБИЛЯ</b>\n\n"
            "🚗 <b>Правила использования:</b>\n"
            "• Вы можете свободно брать и использовать машину согласно вашему абонементу без ограничений\n"
            "• Бензин заправляется сотрудниками транспорта\n"
            "• <b>ВАЖНО:</b> При возврате автомобиля обязательно оставляйте в баке минимум 10 литров топлива\n\n"
            "🛡️ <b>Страховые случаи:</b>\n"
            "• Ущерб, нанесенный автомобилю не по вашей вине, попадает под страховой случай\n"
            "• Ремонт в таких случаях оплачивает организация\n\n"
            "📞 <b>Техническая поддержка:</b>\n"
            "• При возникновении проблем используйте кнопку \"🛠 Техническая поддержка\" в боте\n\n"
            f"🔑 <b>Ваш заказ:</b> {order_data['subscription']}\n"
            f"📅 <b>Действует до:</b> {order_data['end']}\n"
